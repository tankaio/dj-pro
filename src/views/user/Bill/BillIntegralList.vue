<template>
  <div class="bill-integral-list">
    ...
    <x-list position v-model="loading" :finished="finished" :error.sync="error" @load="GetOrders" :active="idx === 0">
      <van-cell v-for="(item, index) in list" :key="index">
        <div class="item">{{ item.time }}</div>
        <div class="item">{{ item.type | filterType }}</div>
        <div class="item">{{ item.amount }}</div>
        <div class="item">{{ item.balan_m }}</div>
      </van-cell>
    </x-list>
    <x-filter-rpop v-model="show_filter_pop" @confirm="filterConfirm" @reset="filterReset">
      <x-filter-item tit="账变类型" type="playMathod integral" bar="bill_type">
        <x-act-button cls="active" :idx="1" v-model="bill_type">现金充值</x-act-button>
        <x-act-button cls="active" :idx="2" v-model="bill_type">充值优惠</x-act-button>
        ...
        <x-act-button cls="active" :idx="5" v-model="bill_type">活动奖励</x-act-button>
        <x-act-button cls="active" :idx="6" v-model="bill_type">购买计划</x-act-button>
        <x-act-button cls="active" :idx="7" v-model="bill_type">彩票投注</x-act-button>
        <x-act-button cls="active" :idx="8" v-model="bill_type">个人撤单</x-act-button>
        <x-act-button cls="active" :idx="9" v-model="bill_type">系统撤单</x-act-button>
        <x-act-button cls="active" :idx="10" v-model="bill_type">打赏(头条)</x-act-button>
        <x-act-button cls="active" :idx="11" v-model="bill_type">打赏(晒单圈)</x-act-button>
      </x-filter-item>
...
<style lang="scss">
.x-filter-item.playMathod.integral {
  .van-button {
    width: 120px;
    margin-right: 15px;
  }
